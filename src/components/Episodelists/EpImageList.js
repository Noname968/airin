      ...
      {epdata?.map((episode) => (
        ...
        >
          <div className='relative w-full flex-1 rounded-lg overflow-hidden bg-[#18181b] aspect-video'>
            <Image src={episode?.img || data?.bannerImage} width={200} height={200} alt={episode?.title} className="bg-[#18181b] h-full w-full object-cover aspect-w-16 aspect-h-9 rounded-lg transition-all duration-300 transform group-hover:scale-105 group-hover:opacity-60" quality={100} />
